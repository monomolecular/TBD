...
import static game.screens.PlayScreen.createTiles;
...
public class CharacterPanel extends JPanel {
    ...
    private int cursorX;
    private int cursorY;
    public static HashMap<Character, BufferedImage> glyphMap;
    private int[][] chars;
    private Color[][] backgroundColors;
    private Color[][] foregroundColors;
    private int[][] oldChars;
    private Color[][] oldBackgroundColors;
    private Color[][] oldForegroundColors;

    private static int charWidth = 9;
    private static int charHeight = 18; //18
    // public static BufferedImage[] glyphArray;
     ...
     */
    public CharacterPanel() throws UnsupportedEncodingException {
        this(80, 24);
    }
     ...
     * @param height
     */
    public CharacterPanel(int width, int height) throws UnsupportedEncodingException {
        super();
        ...
        oldChars = new int[widthInCharacters][heightInCharacters];
        ...
        //glyphArray = new BufferedImage[65536];
    ...
    public void paint(Graphics g) {
        ...
        for (int x = 0; x < widthInCharacters; x++) {
            for (int y = 0; y < heightInCharacters; y++) {
                ...
                try {
                    //BufferedImage img = op.filter(glyphArray[chars[x][y]], null);
                    // The op.filter somehow colors the glyphs.
                    BufferedImage img = op.filter((BufferedImage)glyphMap.get((char)chars[x][y]), null);
                    offscreenGraphics.drawImage(img, x * charWidth, y * charHeight, null);
                } catch (NullPointerException npe) {
                    System.out.println("===START====");
                    System.out.println("Null Pointer Caught!");
                    System.out.println(" - chars[" + x + "][" + y + "] = \'" + (char)chars[x][y] +
                            "\' (" + chars[x][y] + ")");
                    System.out.println(" - glyphmap: " + glyphMap.get(chars[x][y]));
     ...
     */
    public CharacterPanel clear() {
        return clear(Tile.UNKNOWN.glyph(), 0, 0, widthInCharacters, heightInCharacters, defaultForegroundColor, defaultBackgroundColor);
    }
    ...
    public CharacterPanel clear(char character) {
        if (character < 0 || !glyphMap.containsKey(character))
            throw new IllegalArgumentException("character " + character + " must be in the glyphMap: " + glyphMap);
     ...
     * @return this for convenient chaining of method calls
     */
    public CharacterPanel clear(char character, Color foreground, Color background) {
        if (character < 0 || !glyphMap.containsKey(character))
            throw new IllegalArgumentException("character " + character + " must be in the glyphMap: " + glyphMap);
     ...
     * @return this for convenient chaining of method calls
     */
    public CharacterPanel clear(char character, int x, int y, int width, int height) {
        if (character < 0 || !glyphMap.containsKey(character))
            throw new IllegalArgumentException("character " + character + " must be in the glyphMap: " + glyphMap);
     ...
     * @return this for convenient chaining of method calls
     */
    public CharacterPanel clear(char character, int x, int y, int width, int height, Color foreground, Color background) {
        if (character < 0 || !glyphMap.containsKey(character))
            throw new IllegalArgumentException("character " + character + " must be in the glyphMap: " + glyphMap);
    ...
    // ToDo: CharacterPanel Clear and Write method rewrite.
    // They used to take in String and char and I eliminated them all for just String, but the String I was thinking
    // of was actually a String which represented a hex of one character not a string of characters.
     ...
     * @return this for convenient chaining of method calls
     */
    public CharacterPanel write(char character) {
        if (character < 0 || !glyphMap.containsKey(character))
            throw new IllegalArgumentException("character " + character + " must be in the glyphMap: " + glyphMap);
     ...
     * @return this for convenient chaining of method calls
     */
    public CharacterPanel write(char character, Color foreground) {
        if (character < 0 || !glyphMap.containsKey(character))
            throw new IllegalArgumentException("character " + character + " must be in the glyphMap: " + glyphMap);
     ...
     * @return this for convenient chaining of method calls
     */
    public CharacterPanel write(char character, Color foreground, Color background) {
        if (character < 0 || !glyphMap.containsKey(character))
            throw new IllegalArgumentException("character " + character + " must be in the glyphMap: " + glyphMap);
     ...
     * @return this for convenient chaining of method calls
     */
    public CharacterPanel write(char character, int x, int y) {
        if (character < 0 || !glyphMap.containsKey(character))
            throw new IllegalArgumentException("character " + character + " must be in the glyphMap: " + glyphMap);
     ...
     * @return this for convenient chaining of method calls
     */
    public CharacterPanel write(char character, int x, int y, Color foreground) {
        if (character < 0 || !glyphMap.containsKey(character))
            throw new IllegalArgumentException("character " + character + " must be in the glyphMap: " + glyphMap);
     ...
     * @return this for convenient chaining of method calls
     */
    public CharacterPanel write(char character, int x, int y, Color foreground, Color background) {
        if (character < 0 || !glyphMap.containsKey(character))
            throw new IllegalArgumentException("character " + character + " must be in the glyphMap: " + glyphMap);
     ...
     * @return this for convenient chaining of method calls
     */
    public CharacterPanel write(String string) {
        if (string == null)
     ...
     * @return this for convenient chaining of method calls
     */
    public CharacterPanel write(String string, Color foreground) {
        if (string == null)
            throw new NullPointerException("string must not be null" );
     ...
     * @return this for convenient chaining of method calls
     */
    public CharacterPanel write(String string, Color foreground, Color background) {
        if (string == null)
            throw new NullPointerException("string must not be null" );
     ...
     * @return this for convenient chaining of method calls
     */
    public CharacterPanel write(String string, int x, int y) {
        if (string == null)
            throw new NullPointerException("string must not be null" );
     ...
     * @return this for convenient chaining of method calls
     */
    public CharacterPanel write(String string, int x, int y, Color foreground) {
        if (string == null)
            throw new NullPointerException("string must not be null" );
    ...
    public CharacterPanel write(String text, int x, int y, Color foreground, Color background) {
        if (text == null)
            throw new NullPointerException("string must not be null." );
        
        if (x + text.length() >= widthInCharacters)
            throw new IllegalArgumentException("x + string.length() " + (x + text.length()) + " must be less than " + widthInCharacters + "." );
        ...
        for (int i = 0; i < text.length(); i++) {
            write(text.charAt(i), x + i, y, foreground, background);
        }
        return this;
    }
     ...
     * @return this for convenient chaining of method calls
     */
    public CharacterPanel writeCenter(String string, int y) {
        if (string == null)
            throw new NullPointerException("string must not be null" );
     ...
     * @return this for convenient chaining of method calls
     */
    public CharacterPanel writeCenter(String string, int y, Color foreground) {
        if (string == null)
            throw new NullPointerException("string must not be null" );
     ...
     * @return this for convenient chaining of method calls
     */
    public CharacterPanel writeCenter(String string, int y, Color foreground, Color background) {
        if (string == null)
            throw new NullPointerException("string must not be null." );
        ...
        for (int i = 0; i < string.length(); i++) {
            write(Character.toString(string.charAt(i)), x + i, y, foreground, background);
        }
        return this;
    }

    /*
    ToDo: Figure out what was going on with the TileTransformer code.
